...
struct AlertStack: View {
    ...
    private func card(focus: RoadAlert, isPinned: Bool, others: [RoadAlert]) -> some View {
        ...
        return VStack(alignment: .leading, spacing: XRadarSpacing.sm) {
        ...
        .padding(.vertical, XRadarSpacing.sm + XRadarSpacing.hair)
        .frame(maxWidth: .infinity, alignment: .leading)
        .glassEffect(.regular, in: .rect(cornerRadius: XRadarRadius.xl))
        .overlay {
            // A picked alert is outlined in its color; a tap on the card goes back to the nearest.
            if isPinned {
                RoundedRectangle(cornerRadius: XRadarRadius.xl).strokeBorder(accent, lineWidth: 1.5)
            }
        }
        .contentShape(.rect(cornerRadius: XRadarRadius.xl))
        .onTapGesture {
...
private struct FocusLine: View {
    ...
    var body: some View {
        HStack(spacing: XRadarSpacing.md) {
            AlertIconTile(type: alert.type, size: 40, iconSize: 22, radius: XRadarRadius.md)
...
private struct AlertIconTile: View {
    ...
    var body: some View {
        XRadarIconView(icon: type.icon, size: iconSize)
...
private struct VoteRow: View {
    ...
    var body: some View {
        HStack(spacing: XRadarSpacing.sm) {
            button("Toujours là", .check, XRadarColor.accent) { onVote(true) }
            button("Plus là", .close, XRadarColor.textSecondary) { onVote(false) }
        }
    }

    private func button(_ label: String, _ symbol: XRadarSymbol, _ tint: Color, action: @escaping () -> Void) -> some View {
...
private struct AlertChip: View {
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: XRadarSpacing.xs) {
                XRadarIconView(icon: alert.type.icon, size: 16)
                    .foregroundStyle(alert.type.color)
                Text(RoadAlert.distanceLabel(alert.distanceMeters))
    ...
    var body: some View {
        Button(action: action) {
            Text("+\(count)")
                ...
                .padding(.horizontal, XRadarSpacing.sm)
                .padding(.vertical, XRadarSpacing.xs)
                .background(XRadarColor.surfaceHigh, in: .capsule)
        }
        .buttonStyle(.plain)
    }
}
...
private struct AlertListSheet: View {
    ...
    private func row(_ alert: RoadAlert) -> some View {
        Button {
            ...
            HStack(spacing: XRadarSpacing.sm) {
                AlertIconTile(type: alert.type, size: 30, iconSize: 18, radius: XRadarRadius.sm)
                VStack(alignment: .leading, spacing: 0) {
                    Text(alert.title)
                        .font(.xrCallout)
                        .foregroundStyle(XRadarColor.textPrimary)
            ...
            .padding(.vertical, XRadarSpacing.xs)
            .padding(.horizontal, XRadarSpacing.sm)
            .background(alert.key == focusKey ? alert.type.color.opacity(0.10) : Color.clear, in: .rect(cornerRadius: XRadarRadius.md))
            .contentShape(.rect)
        }
        .buttonStyle(.plain)
    }
