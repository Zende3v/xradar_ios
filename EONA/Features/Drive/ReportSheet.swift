...
struct ReportSheet: View {
    ...
    private var picker: some View {
        ...
        .frame(height: Self.tileHeight * 2 + EonaSpacing.md)
        if pages.count > 1 {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: EonaSpacing.lg) {
            ...
            Text("Dans quel sens ?")
                ...
                .foregroundStyle(EonaColor.textSecondary)
            HStack(spacing: EonaSpacing.md) {
                EonaButton(title: "Mon sens", fillWidth: true) { send(ReportDirection.same) }
                EonaButton(title: "Sens opposé", variant: .secondary, fillWidth: true) { send(ReportDirection.opposite) }
            }
            if auto {
                AutoSendBar()
            }
    ...
    var body: some View {
        HStack(spacing: 6) {
            ForEach(0..<count, id: \.self) { index in
                ...
                Circle()
                    .fill(index == current ? EonaColor.accent : EonaColor.borderStrong)
