...
struct ReferralScreen: View {
    ...
    var body: some View {
        Form {
            Section {
                EonaButton(title: "Générer un code (6 mois de membre)", loading: busy, fillWidth: true) {
                    create()
                }
                ...
                if codes.isEmpty {
                    ...
                    ForEach(codes, id: \.code) { code in
                        Button {
                            ...
                            EonaListRow(title: code.code, subtitle: "Créé le \(AccountLabels.shortDate(code.createdAt)) · \(code.months) mois") {
